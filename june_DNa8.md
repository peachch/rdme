...
> __Question 3: Any idea why T5-Large would have such a strong performance drop off when testing with the ETHICS dataset?__
  We admit that we did not spend much effort optimizing the T5 baseline's results. The unstable performance of the ETHICS dataset led to a poor overall average performance. As shown in Table 8, ETHICS contains multiple different subcategories, and we combined data from these categories for binary classification training. The non-representative validation may divided during training and influence the results. However, obtaining a more accurate reason may require more analysis.
